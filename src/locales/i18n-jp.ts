...
Markdownのような構文を使用して、特殊なテキストスタイルを送信できます。

| 語法 Usage | 説明 Description |
| ---- | ---- |
| \\# Heading level 1 | レベル1の見出し  |
| \\#\\# Heading level 2 | レベル2の見出し  |
| \\#\\#\\# Heading level 3 | レベル3の見出し  |
| \\*\\*Bold text\\*\\* | 太字 |
| \\*Italic text\\* | 斜体 |
| \\*\\*\\*Bold italic text\\*\\*\\* | 太字斜体 |
| \\~\\~Delete line\\~\\~ | 取り消し線 |
| \\[color:red;font-size:10px](Font style) | フォントスタイル |

> このフォントの太字は一部のブラウザでは正常に表示されない場合があります。
>
> バックスラッシュ文字 \`\\\` を使用してエスケープすることができます。例：\`\\#\` を入力すると、# がタイトルに変換されずに出力されます。
